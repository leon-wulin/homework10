...
int main() {
  ...
  dsAutoPtr<Balloon>  alice(new Balloon("Hello Kitty")); 
  ...
  // first, without smart pointers
  dsSharedPtr<Balloon> cathy(new Balloon("Buzz Lightyear")); 
  dsSharedPtr<Balloon> daniel(cathy);
  dsSharedPtr<Balloon> elaine(new Balloon("Pokemon")); 
  dsSharedPtr<Balloon> fred(elaine); 
  daniel = fred;
  fred = NULL;
  elaine = cathy;
  cathy = NULL;
